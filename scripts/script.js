...
const tGuide = (key, fallback, vars = {}) => {
...
guide.inspecting = {
  nameKey: "guide.inspecting.title",
  name: "Inspecting",
  descriptionKey: "guide.inspecting.body",
  description: function() {
    return tGuide(
      "guide.inspecting.body",
...
guide.abilities = {
  nameKey: "guide.abilities.title",
  name: "Battle: Abilities",
  descriptionKey: "guide.abilities.body",
  description: function() {
    return tGuide(
      "guide.abilities.body",
      "Abilities are traits that a Pokémon can have. While they are randomized, some abilities can only appear on specific typings. Abilities are sorted in three categories; common, uncommon and rare<br><br>Hidden abilities are innate species-dependent traits that need to be unlocked with an Ability Capsule. Once unlocked, their effect will permanently be active alongside their other ability. Same Hidden Ability and Ability wont stack with each other"
    )
  }
}
...
guide.stab = {
  nameKey: "guide.stab.title",
  name: "Battle: STAB",
  descriptionKey: "guide.stab.body",
  description: function() {
    return tGuide(
      "guide.stab.body",
      "If a Pokémon uses a damaging move that has the same type as one of that Pokémon's types, the move's damage is increased by x1.5<br>This is known as same-type attack bonus, or STAB"
    )
  }
}
...
guide.battleFatigue = {
  ...
  descriptionKey: "guide.battleFatigue.body",
  description: function() {
    return tGuide(
      "guide.battleFatigue.body",
      "Pokémon lose a very small fraction of their maximum health when attacking. This damage can be mitigated by a sum of the HP, Defense and Special Defense of the Pokémon, meaning bulkier Pokémon can battle for longer"
    )
  }
}
...
guide.statusEffects = {
  nameKey: "guide.statusEffects.title",
  name: "Battle: Status Effects",
  descriptionKey: "guide.statusEffects.body",
  description: function() {
    return tGuide(
      "guide.statusEffects.body",
      "Certain moves inflict Status Effects such as {confused}, {burn}, {poisoned}, {freeze}, {paralysis} or {sleep}.<br><br>You can further check their effects by right click/long press<br><br>Status Effects, like temporal stat changes, will count down with turns. You can only apply one Status Effect at a time",
      {
...
guide.weather = {
  ...
  name: "Battle: Weather",
  descriptionKey: "guide.weather.body",
  description: function() {
    return tGuide(
      "guide.weather.body",
      "Certain moves can change the weather to altered ones such as {sunny}, {rainy}, {sandstorm}, {hail}, {foggy}, {electricTerrain}, {grassyTerrain} or {mistyTerrain}<br><br>You can further check their effects by right click/long press<br><br>Altered weathers will last for 15 turns, and only can be changed after 30",
      {
        sunny: tagSunny,
        rainy: tagRainy,
        ...
        hail: tagHail,
        foggy: tagFoggy,
        electricTerrain: tagElectricTerrain,
        grassyTerrain: tagGrassyTerrain,
        mistyTerrain: tagMistyTerrain
      }
    )
  }
}
...
guide.compatibility = {
  nameKey: "guide.compatibility.title",
  name: "Genetics: Compatibility",
  descriptionKey: "guide.compatibility.body",
  description: function() {
    return tGuide(
      "guide.compatibility.body",
      "Compatibility determines how similar the sample is to the host. This influences various parameters such as the chances of inherit, or shiny mutations (only if the sample is shiny)<br><br>Sharing one type with the sample will yield one level of compatibility, whereas sharing two types will increase it by two levels.<br><br>Additionally, if the sample is of the same evolutive line as the host, it will receive maximum compatibility"
    )
  }
}
...
guide.powerCost = {
  ...
  descriptionKey: "guide.powerCost.body",
  description: function() {
    return tGuide(
      "guide.powerCost.body",
      "Power cost determines how taxing it is to modify the host. This influences the time to complete the operation<br><br>This is determined by the division of the host. A higher division will exponentially increase the time required to modify the Pokémon"
    )
  }
}
...
function setGuide(){
  ...
  for (const i in guide){
    ...
    const guideName = tGuide(guide[i].nameKey, guide[i].name)

    div.innerHTML = `<div>${guideName}</div>`
    ...
    document.getElementById("guide-list").appendChild(div)
      ...
      div.addEventListener("click", e => {
        const tooltipName = tGuide(guide[i].nameKey, guide[i].name)
        const tooltipDescription = guide[i].description()

        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").innerHTML = `${tooltipName}`
        document.getElementById("tooltipMid").style.display = `none`
        document.getElementById("tooltipBottom").innerHTML = `<span style="overflow-y:scroll; max-height:25rem; display:inline-block;">${tooltipDescription}</span>`
        ...
        if (i === "stats") {
          ...
          <div class="pkmn-stats-panel" style = "width:100%; justify-content:center; align-items:center;">
                  <div class="pkmn-stats-panel-bst">
                        ...
                        ${tGuide("guide.stats.health", "Health")}
                    </div>
                    <div class="pkmn-stats-panel-bst">
                        ...
                        ${tGuide("guide.stats.attack", "Attack")}
                    </div>
                    <div class="pkmn-stats-panel-bst">
                        ...
                        ${tGuide("guide.stats.defense", "Defense")}
                    </div>
                    <div class="pkmn-stats-panel-bst">
                        ...
                        ${tGuide("guide.stats.specialAttack", "Special Attack")}
                    </div>
                    <div class="pkmn-stats-panel-bst">
                        ...
                        ${tGuide("guide.stats.specialDefense", "Special Defense")}
                    </div>
                    <div class="pkmn-stats-panel-bst">
                        ...
                        ${tGuide("guide.stats.speed", "Speed")}
                    </div>
                </div>
                    `
        }
