...
function setWildPkmn(){
    ...
    document.getElementById(`spiraling-highest-floor`).innerHTML = 
        `<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48">
          <defs>
            <mask id="SVGIz3eBe9X">
              <g fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="4">
                <path d="M8 44h8m-4 0V4"/>
                <path fill="#555555" d="M40 6H12v16h28l-4-8z"/>
              </g>
            </mask>
          </defs>
          <path fill="currentColor" d="M0 0h48v48H0z" mask="url(#SVGIz3eBe9X)"/>
        </svg>
        ${t("explore.spiral.highestFloor", "Highest Floor: {floor}", { floor: saved.maxSpiralFloor })}`
...
function updateItemsGot(){
    ...
    for (const i in item) {
        ...
        if (item[i].type == "tm") divItem.innerHTML = `<img src="img/items/tm${tmTypeIcon(move[item[i].move].type)}.png"> <span>x${item[i].newItem}</span>`;
...
function leaveCombat(){
    ...
    for (const i in item) {
        ...
        if (item[i].type == "tm") divItem.innerHTML = `<img src="img/items/tm${tmTypeIcon(move[item[i].move].type)}.png"><span>+${item[i].newItem}</span>`;
...
function updateItemBag(){
    ...
    for (const i in item) {
        ...
        if (item[i].move && move[item[i].move]) div.innerHTML = `<img src="img/items/tm${tmTypeIcon(move[item[i].move].type)}.png"> <span class="item-list-name">${format(i)} <strong style="opacity:0.6; font-weight:200; white-space:nowrap; font-size:0.9rem; margin-left:0.2rem"> (${move[item[i].move].power} BP, ${format(move[item[i].move].split).slice(0, 3)})</strong> </span>  <span>x${item[i].got}</span>`
...
function updateFrontier() {
    ...
    spiral.innerHTML = `
        ...
        <span class="explore-ticket-left">
        <span style="font-size:1.2rem">${t("help.battle.title", "Battle Tower")}</span>
    ...
    factory.innerHTML = `
        ...
        <span class="explore-ticket-left">
        <span style="font-size:1.2rem">${t("help.factory.title", "Battle Factory")}</span>
